...
class Specter extends Tank {
  ...
  static get botName() { return "Specter"; }
  ...
  _doLock() {
    ...
    const gunDiff = Math.abs(((this.getGunDegree() - this.lockAngle) + 540) % 360 - 180);
    if (!this._hasTarget) {
    ...
    if (gunDiff < aimThreshold && this._hasTarget &&
        this.getCurrentInfo().pturReady && this._shotCD === 0) {
      this.fire();
      this._shotCD = 8;
